...
## Cell-local syntax experiment (2026-09-23)

- JavaScript cells contain ordinary top-level declarations; their variable, destructured, function, class, and enum bindings form the cell's `.value` object. Nested/local declarations do not export. The reactive graph still tracks whole cells, not individual bindings. Callback helpers are unsupported, including in previously saved documents.
- A JavaScript cell consisting of one expression publishes that expression's value instead of an empty exports object. Reads still cross the handle boundary explicitly: `parent.budget.value.total`, not `parent.budget.total`. The fast preparer returns the transpiled expression; the semantic wrapper infers that expression's type. Existing compiled cache records were invalidated for this change.
- Markdown cells require a single `md` tagged template, e.g. ``md`Total: ${parent.budget.value.total}```. Markdown interpolation produces source text that continues through the existing marked/DOMPurify rendering path.
- `root`, `parent`, and `self` are lexical parameters of each trusted page-realm execution, not mutable properties of `globalThis`. Fast dependency analysis and the semantic virtual project recognize the same implicit handles. Top-level declarations with those names conflict with the injected bindings; choose different local names.
- New empty calculations start with `const value = 0`, and new live Markdown cells start with an empty `md` tagged template. Short prose carried into a calculation becomes a quoted `const value = "…"`; prose carried into Markdown becomes an escaped tagged template, so literal backticks and `${...}` cannot become executable interpolation. The cache compatibility key was advanced so old compiled results are not reused under these new semantics.

## Current state

Commits 1–10 and the local-first document milestone are complete. A personal-document pass then added note splitting, indent and outdent, cell-boundary keyboard movement, and kind conversion. The app still starts with a blank document instead of the deterministic Tiny Commerce fixture. A separate IndexedDB database holds the version-1 source document independently of the bounded compiler/output cache. Source and structure edits autosave in order; JSON import/export and document-level undo/redo are available. Imported executable code remains paused until an explicit Run all, while the current page-realm execution boundary remains unsandboxed. The Tiny Commerce fixture stays in source and tests.
...
- A note splits at the cursor with Enter. The text before the cursor stays; the text after becomes the next note. A cursor sitting in the blank between paragraphs does not leave an empty line in either note. The root has no siblings, so its split becomes its first child; every other split becomes the next sibling, and that note's previous children move with the text that left. Shift+Enter inserts a newline and stays in the note. Keystrokes that arrive before the new editor can take focus are held and delivered to it.
- Tab and Shift+Tab indent and outdent a note while writing. Alt+Right/Left and Mod+]/[ do the same from a calculation, where Tab still indents code. Indent moves the cell under the visible previous sibling and expands that sibling if it was collapsed. Outdent moves it after its parent. The root cannot indent, and a child of the root cannot outdent.
- ArrowUp at the start of a cell and ArrowDown at its end move into the neighboring visible cell. They do nothing at the ends of the visible tree.
- A cell other than the root can be deleted from its row, or with Backspace or Delete while the outline row itself is focused. Deletion removes that cell and everything nested in it, asks first, and moves focus to the previous sibling or the parent. Undo restores the subtree. Backspace inside an editor still deletes text.
- The kind control on a cell turns it into a note, a calculation, or a live Markdown view. A short single-paragraph note becomes a quoted `const value = "…"` or escaped `md` template; the prose is not parsed into numbers. A long note, or a note with more than one paragraph, starts from the empty program so a whole notebook is not silently quoted. Adding a calculation beneath a short note carries that same quote. The root stays a note. Executable cells still run only after an explicit Run all when execution is paused.
...
- `src/runtime/analyze-dependencies.ts` owns the TypeScript compiler API import. It analyzes JS declarations/expressions and Markdown templates without executing code, preserves half-open source offsets, respects lexical shadowing including switch `CaseBlock` scope, reports computed or aliased handles, de-duplicates resolved target IDs, and leaves source unchanged. Scalar handle metadata and chains beyond runtime `.value` data are not misclassified as handle aliases or dependency paths; nested `.value` reads stop at the first runtime-value boundary. Text cells have no executable dependencies, and syntax/path failures remain per-cell issues.
...
- `src/compiler/fast-prepare.ts` owns syntax validation and transpilation for execution. Its worker-local cache reuses output only for byte-identical cell ID, kind, and source while every document revision rebuilds dependency/path meaning. Fast output carries provisional `string`, `unknown`, invalid, and cycle type/status data plus serialized graph data and measured preparation counters.
...
- Prose preview and editor layers remain mounted in the same CSS grid area. The inactive layer uses only `visibility` and `pointer-events`, so both layers continue contributing to one stable host height while preview/edit state changes. Note splitting and keyboard movement act on the editor layer, which is the one that holds the caret.
